...
def BuildModel(IMAGESasFEATURES, LABELS):
    ...
    customModel = keras.Sequential([
        ...
        keras.layers.Dense(units = hiddenUnits, activation='relu', kernel_initializer= 'VarianceScaling', use_bias= True),
        keras.layers.Dense(units = numClasses, activation='softmax', kernel_initializer= 'VarianceScaling', use_bias= False)
    ])
    ...
    # customModel.summary()
    customModel.fit(IMAGESasFEATURES, LABELS, batch_size=calcBatchSize, epochs=epochs)
    return customModel
